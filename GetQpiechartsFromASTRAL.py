...
### To run the script on a remote server type "xvfb-run python GetQpiechartsFromASTRAL.py" in the terminal
### Locally "python GetQpiechartsFromASTRAL.py" should be enough
### You may first have to indicate the path to ete3, for instance by typing "export PATH=/home/user/anaconda_ete/bin:$PATH"
#############################################################################

### The script starts here:

from ete3 import Tree, TreeStyle, TextFace, NodeStyle, PieChartFace, faces

### import tree (change input file name as required)
t = Tree( "SpeciesTree_BP10_annotQ3.tre" )
...
t.set_outgroup("outtaxon")

### Nothing more to change below this line EXCEPT the name of the output file at the end (and except if you are interested in making pies with posterior probabilities intead of quartet support, then see comments below).
...
for n in t.traverse():
   ...
   if hasattr(n,"q3"):
      ...
      Qtot = Q[0] + Q[1] + Q[2]
      print str(Qtot)
      qpie = PieChartFace(percents=Q, width=10, height=10, colors=["red", "cyan", "gray"], line_color=None)
...
### export picture (change name as you wish)
t.render("SpeciesTree_BP10_annotQ3.png", w=183, units="mm", tree_style=ts)
